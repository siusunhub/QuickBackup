...
namespace quickbackup
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            Text = $"QuickBackup {AppVersion}";
            trayIcon.Text = $"QuickBackup {AppVersion}";
            ...
            chkAutorun.CheckedChanged += (_, _) =>
            {
                if (!loadingSettings)
                {
                    SetAutorun(chkAutorun.Checked, true);
                }
            };
        ...
        private void LoadSettings()
        {
            ...
            try
            {
                ...
                SetAutorun(config.AutoRunAtStartup, false);
        ...
        private void SetAutorun(bool enabled, bool save)
        {
            try
            {
                ...
                if (enabled)
                {
                    ...
                    Log($"Enabled autorun at startup: {Application.ExecutablePath}");
                }
                else
                {
            ...
            {
                Log($"Could not update autorun setting: {ex.Message}");
            }
        }
        ...
        {
            Log("Memory collection started.");
            GC.Collect();
            GC.WaitForPendingFinalizers();
            GC.Collect();
            Log("Memory collection finished.");
        }
